// tsc ./utils.ts --noEmit && ts-node ./utils.ts
// 
...
function log<This, Args extends any[], Return>( // Added types: class types, arguments, and return types
  target: (this: This, ...args: Args) => Promise<Return>,
  context: ClassMethodDecoratorContext<
    This,
    (this: This, ...args: Args) => Promise<Return>
  ...
  const resultMethod = async function (this: This, ...args: Args): Promise<Return> {
    console.log(`@log - Running the ${context.name.toString()} method`) // toString (b/c initially allowed number | string)
    try {
      return await target.apply(this, args)
    } finally {
...
// Implements a decorator that calculates duration of method execution
class Metric { 
  constructor(public name: string) {} // name of metric that want to measured

  // Method returning a decorator
  time() {
    const metricThis = this // Record reference to metric object
    // Decorator
    return function (target: any, context: any) {
      ...
      const resultMethod = async function (this: any, ...args: any[]) {
        ...
        try {
          return await target.apply(this, args)
        } finally { // Regardless if method fails/succeeds, will still get message
          const end = Date.now()
          const timeMs = end - start // Compute time from the difference of end/start time
...
function createMetric(name: string) { // Creates a metric that receives name and returns instance of Metric class
  return new Metric(name)
}

const weatherTiming = createMetric('weather.timing') // Creates instance
...
class WeatherAPI {
  ...
  @weatherTiming.time() // Makes use of multiple decorators
  @logAndRetry // time decorator then retry --> Gives duration for each retry
